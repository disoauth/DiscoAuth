from DisOAuth import AuthUrl as auth, permissions

TEST_DIS_APP_ID = "1172638888553103371"

async def test_url_build():
    url = await auth(TEST_DIS_APP_ID, ["identify", "guilds"], "https://tests.com", test=True).makeUrl()
...
async def test_perm_build():
    perms = permissions(3) # Check permissions_test.py for the permissions tests
    url = await auth(TEST_DIS_APP_ID, ["identify", "guilds", "bot"], "https://tests.com", perms, test=True).makeUrl()
